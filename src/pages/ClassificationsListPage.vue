<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Classifications</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="classification-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(classifications ?? []) as unknown as Classification[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToClassification"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="classification-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load classifications: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Classifications List Page - Showcase of impact_spa_utils simplicity
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Classification>({
  queryKey: ['classifications'],
  queryFn: (params) => api.getClassifications(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToClassification(_event: unknown, { item }: { item: Classification }) {
  router.push(`/classifications/${item._id}`)
}
